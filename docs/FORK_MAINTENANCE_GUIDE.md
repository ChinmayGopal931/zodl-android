...
> **Purpose**: Single reference for maintaining `zodl-android-fork` (Zapp) relative
> to `zodl-android` (upstream Zodl/Zashi). Created 2026-05-20.
>
> **Golden rule**: `zodl-android` is READ-ONLY. All edits happen in `zodl-android-fork`.
...
```
zodl-android (upstream Zodl/Zashi)        zodl-android-fork (Zapp)
====================================      ====================================
...
├── zodl-android-fork/        # Zapp — all edits here
...
├── BLIND_PEER_DEBUG.md                 ★ Blind peer debugging
├── design/                             ★ Zapp design assets (HTML, JSX, SVGs)
├── zkp2p-providers/                    ★ ZKP2P provider definitions
│   ├── providers.json                    Registry (PIX, UPI, GCash, M-Pesa)
│   ├── pix/transfer_pix.json             Brazil (draft)
│   ├── upi/transfer_upi.json             India (draft)
│   ├── gcash/transfer_gcash.json         Philippines (draft)
│   └── mpesa/PENDING.md                  Kenya/TZ/UG/GH (pending)
├── app/
│   └── src/main/res/xml/
...
### 4.6 Fiat On/Off-Ramp (peer.xyz)

`PeerXyzUtil.kt`:
- Onramp: `https://www.peer.xyz/?recipientAddress={addr}&toToken=ZEC&referrer=Zapp`
- Offramp: `https://www.peer.xyz/sell?amount={amt}&referrer=Zapp`
- Fallback: `https://justzappit.xyz/directory` for unsupported regions
- Unsupported: BR, IN, PH, KE, TZ, UG, GH (waiting for ZKP2P)

Entry points: Home "Get ZEC", zero-balance CTA, insufficient-funds sheet, More > Cash out,
transaction detail "Convert".
...
### 4.12 ZKP2P Providers

`zkp2p-providers/providers.json` — registry of payment rail providers:
- PIX (Brazil) — draft
- UPI (India) — draft
- GCash (Philippines) — draft
- M-Pesa (Kenya/TZ/UG/GH) — pending

### 4.13 AndroidManifest Changes
...
| `ui-lib/build.gradle.kts` | +chat/peer_onramp/offramp/unified_send res sets, -keep_open/voting, +coil-compose/play-services-location/zappmessaging deps, -zcash-sdk-backend/kotlin-test/ktor-client-mock |
...
| `SDK_INCLUDED_BUILD_PATH` | (empty) | `../zcash-android-wallet-sdk` | Fork uses local SDK |
...
| `CLAUDE.md` | Root | Architecture overview, key files |
| `ZAPP_UI_SCREEN_MAP.md` | `docs/` | Complete UI screen inventory |
| `ZAPP_CLEANUP_REPORT.md` | `docs/` | Dead code analysis |
| `HANDOVER_UI_CLEANUP.md` | `docs/` | UI cleanup notes |
| `chat-refactor-playbook.md` | `docs/` | Chat refactoring guide |
