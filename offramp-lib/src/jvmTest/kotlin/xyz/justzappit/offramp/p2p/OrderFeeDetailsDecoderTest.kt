...
class OrderFeeDetailsDecoderTest {
    ...
    @Test
    fun `decodes a populated tuple (post-completion shape)`() {
    ...
    @Test
    fun `timestamps treated as null when zero (contract default for unwritten fields)`() {
    ...
    fun `getAdditionalOrderDetailsCalldata uses correct selector and arg encoding`() {
        ...
        val calldata = DiamondCalls.getAdditionalOrderDetailsCalldata(java.math.BigInteger.valueOf(42))
        ...
        assertEquals(
            ...
            tail.joinToString("") { "%02x".format(it) },
        )
    }
}
