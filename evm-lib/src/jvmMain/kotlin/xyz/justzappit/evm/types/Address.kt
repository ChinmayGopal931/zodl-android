...
/**
 * An EVM-style 20-byte address. Constructed only through validating factories; the stored form is
...
class Address private constructor(
    ...
    companion object {
        ...
        internal fun toEip55(lowerHex: String): String {
            val hashHex = keccak256(lowerHex.toByteArray(Charsets.US_ASCII)).toHex()
            return buildString(lowerHex.length) {
                for (i in lowerHex.indices) {
                    ...
                    if (c.isDigit()) {
                        ...
                        append(if (nibble >= EIP55_THRESHOLD) c.uppercaseChar() else c)
                    }
                }
            }
        }
        ...
        // Declared after HEX_CHARS so its initializer (which calls parse → parseOrNull → HEX_CHARS)
        // sees the populated set; Kotlin companion-object members init in declaration order.
