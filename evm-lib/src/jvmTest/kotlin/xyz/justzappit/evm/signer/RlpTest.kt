...
class RlpTest {
    ...
    fun `dog string encodes per yellow paper`() {
        assertEquals("83646f67", Rlp.encode(rlpBytes("dog".toByteArray())).toHex())
    }
    ...
    fun `integer 1024 encodes as 0x820400`() {
        assertEquals("820400", Rlp.encode(rlpInt(BigInteger.valueOf(1_024))).toHex())
    }

    @Test
    fun `negative integers are rejected`() {
        assertFailsWith<IllegalArgumentException> {
            rlpInt(BigInteger.valueOf(-1))
        }
    }

    @Test
    fun `long string (gt 55 bytes) uses long-form length prefix`() {
        val s = "Lorem ipsum dolor sit amet, consectetur adipisicing elit"
        val encoded = Rlp.encode(rlpBytes(s.toByteArray())).toHex()
        assertEquals("b8" + "38" + s.toByteArray().toHex(), encoded)
    }
}
