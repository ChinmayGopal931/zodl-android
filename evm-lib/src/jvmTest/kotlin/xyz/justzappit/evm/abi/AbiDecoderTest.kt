...
class AbiDecoderTest {
    ...
    fun `uint reads a word as an unsigned big-endian integer`() {
        ...
        assertEquals(BigInteger.ZERO, d.uint(0))
        assertEquals(BigInteger.valueOf(258), d.uint(1))
    }
    ...
    fun `word returns the raw 32 bytes`() {
        ...
        assertEquals(BigInteger.valueOf(2), BigInteger(1, d.word(1)))
    }
    ...
    private companion object {
        ...
        fun uintWord(value: Long): ByteArray {
            ...
            return ByteArray(WORD).also { System.arraycopy(src, 0, it, WORD - src.size, src.size) }
        }
