...
internal class GetUpiOfframpRateUseCase(
    private val rpc: BaseRpcClient,
    private val network: P2pNetworkConfig,
) {
    suspend operator fun invoke(currency: CurrencyCode): BigDecimal? =
        runCatching {
            val raw = rpc.ethCall(
            ...
            Twig.warn(it) { "GetUpiOfframpRateUseCase: getPriceConfig(${currency.code}) failed" }
        }.getOrNull()
}
