...
import xyz.justzappit.offramp.p2p.UpiQrError
import xyz.justzappit.offramp.p2p.UpiQrParseResult
import xyz.justzappit.offramp.p2p.UpiQrParser

/**
 * UPI-specific scan VM. Mirrors [co.electriccoin.zcash.ui.screen.scan.ScanGenericAddressVM] but
 * routes the scanned payload through [UpiQrParser] (which is byte-compatible with `@p2pdotme/sdk`
 * v1.1.7) before declaring the scan VALID. A merchant QR that doesn't pattern-match the SDK's
 * regex would fail downstream inside `placeOrder`/`setSellOrderUpi`, so we reject early.
 */
